...
**Spine — "refuse the aggregate" (1 of 4).** This lesson opens the program's longest-running argument, and each later lesson re-runs it at higher resolution: here **the average lies** when variance is high or outliers dominate; in `LSN-0.3` **the aggregate lies** when subgroups differ (91% overall is 95% and 32%); in `LSN-0.4` **the confidence lies** when the spread varies by region; in `LSN-1.3` **the MAE lies** when the error concentrates (a 38-hour headline hiding 330-hour misses). Say the spine's name out loud in segment 3 — participants who hear it four times across two modules leave with a reflex rather than four anecdotes.
...
| Sampling debrief + bias | 15 min | discussion | Everyone ran the n = 30 / 3,000 / 30,000 simulation in pre-work task 3, so this is the debrief, not the demo: collect what they saw, put two of their plots on screen, name the effect. Then the part that needs a room, because it is judgement and not arithmetic — bias, which no sample size fixes: an NPS survey only angry users answer; latency measured only during EU business hours; Wald's WWII bombers (armor where the returning planes *weren't* hit). **Then name the six types with commercial tells (`SUP-12` §1), because "watch out for bias" is unactionable and a named list is checkable:** reporting (tickets record complaints, not satisfaction) · selection (the CRM holds deals someone bothered to log) · temporal (pre-reorganization data, post-reorganization estimates) · in-group (estimators rate familiar-looking work as low risk) · out-group homogeneity (every client in an unfamiliar sector scored on one profile) · confirmation (the analysis rerun until it agrees with the pitch). Close on the group drill: *"18 months of won and lost deals, exported from the CRM"* — which of the six are in there? (At least three, and they find them.) Land the two questions for any number: how many, and collected how? |
| Variance and outliers: when the average lies | 13 min | discussion | Kept live because it is a decision, not a computation: two delivery teams, both mean cycle time 5 days, Team A σ = 0.5 days vs Team B σ = 4 days — whose estimate do you put in a client commitment, and what do you write in the SOW? Reveal the $2M whale dragging "average deal size" from $80k to $208k while the median barely moves. Fixes named: median, percentiles, plot before you summarize. **Then give "that point looks wrong" a rule, so the exclusion is a method and not an opinion: the 1.5 × IQR fence** — Q1, Q3, IQR = Q3 − Q1, anything outside Q1 − 1.5·IQR or Q3 + 1.5·IQR is flagged, which is exactly what a box plot draws. The rule flags the point; it does not tell you what to do with it — sometimes the outlier is bad data and sometimes it is the entire business case. *Rescope: the recompute arithmetic and Anscombe's quartet moved to homework task 2 — here they are shown as results, not worked* |
...
| # | Task | Time | Artifact |
|---|---|---|---|
| 1 | The schedule-slip notebook (from the GAP-1 exercise notebook): a provided 32-row dataset of schedule slip in days from past fixed-bid projects, containing one +45-day outlier. (a) compute mean, median, variance; (b) plot the histogram; (c) **identify the outlier by the 1.5 × IQR fence rather than by eye**, recompute without it, and state which rule you applied; (d) write two sentences — would you trust a model trained on these 32 points to predict overrun for a new project, and what one thing about *how this sample was collected* would you check first? | 30 min | Executed notebook + the fence values + the two sentences |
| 2 | **Relocated from live segment 3:** work the two arithmetic pieces the session now only shows as results — rebuild the 15-deal whale table (14 deals averaging exactly $80k + one $2M) and confirm the mean moves 2.6× while the median holds. Then Anscombe's quartet, **staged as a prediction you commit to before you plot**: the notebook shows only the summary statistics first (mean x = 9, variance x = 11, mean y = 7.5, variance y ≈ 4.13, correlation ≈ 0.816, fit y = 3 + 0.5x — identical to two significant figures; the reveal shows the third decimal) and asks you to sketch or describe what the data looks like — *then* it plots all four datasets. Write one line on what the identical statistics hid, and one on how close your prediction was | 25 min | The deal table with both statistics + your written prediction + the quartet plot + two lines |
...
| Supplement — log transform for wide-ranging values + the 1/√n sampling law (optional homework extensions) | supplement | [`program/sources/SUP-2-data-reality-catalog.md`](../sources/SUP-2-data-reality-catalog.md) §4–5 |
| Supplement — the decision case (a chart that lost the argument), Anscombe restaged as predict-then-reveal, the 1.5 × IQR fence rule, chart rules, and the palette accessibility check | supplement | [`program/sources/SUP-10-charts-that-communicate.md`](../sources/SUP-10-charts-that-communicate.md) |
| Supplement — six named bias types + the CRM-export drill for the sampling-bias segment | supplement | [`program/sources/SUP-12-bias-and-disaggregation.md`](../sources/SUP-12-bias-and-disaggregation.md) §1 |
| Supplement — varying spread as the model-output half of "when the average lies" (forward reference to LSN-0.4 and LSN-1.3) | supplement | [`program/sources/SUP-11-uncertainty-decomposition.md`](../sources/SUP-11-uncertainty-decomposition.md) §3 |
