...
**Spine — "refuse the aggregate" (3 of 4).** `LSN-0.2`: the average lies when the spread is wide. `LSN-0.3`: the aggregate lies when subgroups differ. Here: **the confidence lies when the spread varies by region** — a system that is tight on standard cases and wide on unusual ones reports an average error that describes neither, and the client sentence is *"it is accurate for your standard jobs and unreliable for your unusual ones."* `LSN-1.3` closes the spine on MAE. Segment 2 is the natural place to name it, because the SOW criterion is what the spine is for.
...
|---|---|---|---|
| The three-bucket sort, then the second axis | 25 min | discussion | Participants' pre-work examples go on a shared board, sorted live into deterministic / probabilistic / stochastic. Anchors: deterministic — `2+2`, a SQL `SUM`, a build with pinned deps (same input, same output, every time); probabilistic — "spam, confidence 0.87": a guess with stated uncertainty; stochastic — randomness in the process itself: retry jitter, load-balancer routing, SGD training, LLM sampling at temperature > 0. Debate one misfiled favorite: an LLM at temperature 0 (near-deterministic in practice, not contractually — batching and floating-point effects). **Then re-sort the same board on a second axis, which is the one clients actually pay for: *would more data fix this?*** Irreducible — variation in the process itself; more data will not remove it, and the honest sentence is "this much variation is real and will not go away." Reducible — the model has not seen enough, or is the wrong shape; more and better data shrinks it, and you can say roughly by how much. That single question is the credible version of "how much data do we need?" (`LSN-1.6`), and it is why the `LSN-1.4` degradation curve flattens instead of reaching 100% — the reducible part gets eaten and the irreducible part does not. *Swap note (2026-08-11): the second axis is bought from two of the three misfiled-favorite debates ("flaky test" and hash function, both now anchors rather than debates) — segment stays 25 min* |
...
| 3 | **Two halves of one acceptance criterion — how many questions, and is the confidence honest.** (a) *Eval-set sizing* (notebook's acceptance-arithmetic cell): a genuinely-92% system faces a "≥ 90% of the test set judged correct" bar. Compute the test-set size that makes it pass 19 runs in 20, and state what you would tell a client who wants the bar set on 50 questions. (b) *Calibration check* — take the notebook's provided prediction set, bucket every prediction by the confidence it stated (50–60%, 60–70%, …), compute the fraction actually correct in each bucket, and plot stated against observed. A calibrated system sits on the diagonal; below it is overconfident, which is the common case. **This is the name for the overconfident-twin prop you saw in segment 1** — two scorers making identical decisions where one's confidence is inflated by 9.6 points. Write the acceptance criterion it enables: *"calibrated within X points across confidence buckets"* — a criterion a client can check, rather than one they have to trust | 20 min (12 + 8) | Your n, the arithmetic, and the client sentence · the reliability plot and the one-line calibration criterion |
...
| Supplement — why a generative system's acceptance criterion must be statistical (justification for the homework's SOW criterion) | supplement | [`program/sources/SUP-7-failure-and-trust-catalog.md`](../sources/SUP-7-failure-and-trust-catalog.md) §4 |
| Supplement — reducible vs irreducible uncertainty and the "will more data fix this?" question; varying spread; **calibration** (names the overconfident-twin prop, LL-23) as a candidate acceptance-criterion swap; out-of-depth detection | supplement | [`program/sources/SUP-11-uncertainty-decomposition.md`](../sources/SUP-11-uncertainty-decomposition.md) |
