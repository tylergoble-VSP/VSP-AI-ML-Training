...
| 2 | **The graded artifact, staged as write-your-own-shortfalls-then-fix-them** *(reframed 2026-08-11 — same 45 min; see `SUP-5`, "The teaching sequence worth stealing")*. Re-point the pipeline at a different small corpus — your own project docs (anonymized) or the provided fallback pack. Then, **in this order**: (a) query it and write **your own** bulleted list of its shortfalls, with one line each on why you think that shortfall happens — before reading any fix list; (b) apply one or two fixes from the optimization ladder (`SUP-5`) — the top-*k* distance cut-off (knob 10) is two lines and usually visible, and a chunking-strategy change (knob 2) is the next cheapest; (c) **return to your own bullet list and state, per bullet, whether it moved.** *Grading standard:* pipeline runs end-to-end; the shortfall list is specific enough to be checkable (not "retrieval could be better"); at least one bullet is honestly marked unmoved. **Naming your own failures before being told what they are called is where the judgement forms** — a fix list handed over first turns this into a knob tour | 45 min | Notebook + the before-and-after shortfall list, submitted referencing `LSN-2.4` |
...
| Supplement — the twelve-knob RAG optimization ladder: names the knob behind each scripted failure demo, adds the top-k cut-off as a homework grid axis, and supplies the retrieval-vs-generation diagnostic order | supplement | [`program/sources/SUP-5-rag-optimization-ladder.md`](../sources/SUP-5-rag-optimization-ladder.md) |
