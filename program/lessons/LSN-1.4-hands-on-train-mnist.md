...
| Evaluate: accuracy + confusion matrix | 30 | lab + drill | **Where:** §"Evaluation" (adapted from `02` §Evaluation). Compute held-out test accuracy (expect ≥95%), build the 10×10 confusion matrix with labeled axes. Reading drill, LSN-1.3 callback — checkpoint questions asked live: "Which digit does your model most mistake for a 9?" "Is that a precision or a recall problem for class 9?" "Would you ship this for reading handwritten invoice totals? What's the cost of one 4→9 error?" **Lab checkpoint:** everyone has test accuracy + a rendered confusion matrix and can name their model's worst digit pair (typically 4/9 or 3/5). **Likely failure:** evaluating on training data ("I got 99.8%!") or transposed matrix axes. **Recovery:** the notebook's plotting helper hard-labels true-vs-predicted axes; suspicious scores get the "which loader did you use?" question first. |
...
|---|---|---|
| Guided fill-in MNIST lab notebook (PyTorch MLP on MNIST: MLP framing from source 01 + data/train/eval pipeline from source 02; new "Break It" section; `fresh_model()` helper; synthetic fallback) | build (**GAP-8**) | `notebooks/neural_networks/` (new file, adapted) |
...
| Pre-downloaded MNIST fallback (`mnist_784.npz`) on shared drive | build (part of GAP-8) | shared drive — location set at delivery prep |
